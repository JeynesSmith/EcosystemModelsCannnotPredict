...
% Lasso method and a cutoff. Optimal parameters for Lasso and the cutoff
% were selected to be lasso=1 and cutoff=0.001. This script only fits to unique combinations of
% species in the microcosm dataset.

LassoVals = [10, 1, 0.1, 0.01 0.001 0.0001];
CutOffVals = [0.1 0.01 0.001 0.0001];
for i = 2%1:6
for j = 3%1:4

Target = 1000; % number of simulations for each experiment
load 'Experimental data'/ProposedTargets_Unique TargetExperiments % [1 11 19 26 30 31]
%TargetExperiments = TargetExperiments([30,31,33:35]);
...
save('Experimental fitting results/Results_LassoOptimal','FittingResults','Target','TargetExperiments','LassoParameter','CutOffParameter', '-v7.3') % save parameters and initial storage
...
% save('FitResults\Results_Lasso1','FittingResults','Target','TargetExperiments', '-v7.3') % save parameters and initial storage
end
pause(1)
...
% LassoParameter = 0.01; % This parameter affects the strength of the lasso method which minimises paras
...
LB = -A_amp.*ones(NumSpp*(NumSpp+2),1); % lower bound
UB = +A_amp.*ones(NumSpp*(NumSpp+2),1); % upper bound

% No negative initial conditions OR growth rates allowed in the search
LB(1:2*NumSpp) = 0;
% LB(1:NumSpp) = 0; % allowing negative growth
...
while NumSets < Target % loop until Target is reached
    ...
    % r_g = 2.*A_amp.*(rand(NumSpp,1)-0.5); % initial guesses for the growth rates % allowing negative growth
    ...
    [V_fit,SSD] = fmincon(@(x) FitNoisyData(x,Tr,nr,n_max,NumSpp,LassoParameter,CutOffParameter),V0,[],[],[],[],LB,UB,[],options);
    ...
    if SSD < SSD_bar
        ...
        TempStorage{NumSets,5} = reshape(V_fit(2*NumSpp+1:NumSpp*(NumSpp+2)),NumSpp,NumSpp); % Interaction matrix
...
function [SSD,t_q,n_q] = FitNoisyData(V,t_n,n_n,n_max,NumSpp,parameter,CutOffParameter)
...
opts = odeset('RelTol',1e-3,'Events',@(t,n) AbundEvent(t,n,n_max));
[t_q,n_q] = ode45(@(t,n) species_DE(t,n,reshape(V(2*NumSpp+1:end),NumSpp,NumSpp),V(NumSpp+1:2*NumSpp)),t_n,V(1:NumSpp),opts);
