% This script generates supplementary fits to microcosm data using a log 
% likelihood fit statistic. This script only fits to unique combinations of
...
save('Experimental fitting results/Results_LogLikelihood','FittingResults','Target','TargetExperiments', '-v7.3') % save parameters and initial storage

% loop over microcosm experiments
parfor Index = 1:length(TargetExperiments)
    disp(Index)
    [A] = sub_Multiple_Experimental_Fits(TargetExperiments(Index),Target); % run the fitting procedure
...
% fitting procedure
function [TempStorage] = sub_Multiple_Experimental_Fits(DS,Target)
...
while NumSets < Target % loop until Target is reached
    ...
    [V_fit,SSD] = fmincon(@(x) FitNoisyData(x,Tr,nr,n_max,NumSpp),V0,[],[],[],[],LB,UB,[],options);
    ...
    if SSD < SSD_bar
        ...
        disp(['For dataset #' num2str(DS) ': Number ' num2str(NumSets) ' found with ln(L)=' num2str(SSD) '; ' num2str(count) ' searches.'])
    end
end
end
% data fiting function
function [SSD,t_q,n_q] = FitNoisyData(V,t_n,n_n,n_max,NumSpp)
...
if size(n_n,1) ~= size(n_q,1)  % if vector lengths do not match (because of parameter problems e.g. infinite growth/death), set SSD to be large
    ...
    SSD = -1*sum(log(normpdf(n_q(:),n_n(:),V(end)))) + V(end); % log likelihood
end
end
