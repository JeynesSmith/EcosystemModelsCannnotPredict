...
%             obj.MedianPlot = scatter(pos, quartiles(2), args.MarkerSize, [1 1 1], 'filled');
            ...
            medianDensityWidth = interp1(value(unique_idx), density(unique_idx), medianValue, 'linear','extrap')*width;
            if medianDensityWidth<args.BoxWidth/2
                medianDensityWidth=args.BoxWidth/2;
            end
            switch args.HalfViolin
                case 'right'
                    obj.MedianPlot = plot(pos+[0,1].*medianDensityWidth, ...
                        [medianValue, medianValue]);
                case 'left'
                    obj.MedianPlot = plot(pos+[-1,0].*medianDensityWidth, ...
                        [medianValue, medianValue]);
                case 'full'
                    obj.MedianPlot = plot(pos+[-1,1].*medianDensityWidth, ...
                        [medianValue, medianValue]);
            end
